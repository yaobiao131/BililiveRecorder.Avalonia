...
#pragma warning restore CS0067 // The event 'Recorder.PropertyChanged' is never used
    ...
    public IRoom AddRoom(long roomid, Platform platform) => this.AddRoom(roomid, true, platform);

    public IRoom AddRoom(long roomid, bool enabled, Platform platform)
    {
        lock (this.lockObject)
        {
            this.logger.Debug("AddRoom {RoomId}, AutoRecord: {AutoRecord}", roomid, enabled);
            var roomConfig = new RoomConfig { RoomId = roomid, AutoRecord = enabled, Platform = platform };
